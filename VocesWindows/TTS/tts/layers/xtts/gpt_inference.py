    ...
    def forward(
        ...
        **kwargs,
        ...
        transformer_outputs = self.transformer(
            ...
            # Always False for the INNER call (E015, 2026-08-29): GPT2Model.forward rebuilds a
            # 26-entry tuple (one per layer + final) via `tuple + (tensor,)` on EVERY decode step
            # when True -- real Python overhead repeated on every autoregressive token. The only
            # consumer of the outer `hidden_states` field (stream_generator.py's sample_stream)
            # reads only hidden_states[-1], which (verified against this exact transformer source)
            # is appended AFTER the final layer norm, i.e. it IS `transformer_outputs[0]` --
            # already computed below as `hidden_states`, at zero extra cost. So the outer contract
            # is preserved (still a 1-tuple ending in the right tensor) while skipping the
            # expensive inner tuple-building entirely, regardless of what output_hidden_states the
            # caller requested.
