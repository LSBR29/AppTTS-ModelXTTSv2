...
from TTS.tts.layers.xtts.prealloc_cache import build_preallocated_cache
...
# --- bounded-left-context streaming re-vocoding (experiment E003) ------------------
# Left context, in GPT latent tokens, kept in front of each new streaming chunk when
# inference_stream re-vocodes. Measured requirement is ~4 tokens: the HiFi-GAN
# generator's own left receptive field is 12.66 generator-input frames
# (.claude-work/experiments/E003_quadratic_revocode/results/rf_measure_*.json), which
# is ~2.9 tokens at 4.354 frames/token, plus ~1.25 tokens for the two F.interpolate
# stages; an end-to-end measurement through the ONNX graph
# (results/boundary_decay_*.json) shows the difference against a full from-token-0
# re-synthesis reaching its float32 numerical floor (~-75 dB) at exactly 4 tokens
# and not improving beyond that. 16 gives ~4x margin.
XTTS_STREAM_LEFT_CONTEXT_TOKENS = 16


def _stream_ctx_lattice(decoder):
    """(lattice_tokens, samples_per_lattice_step) for bounded-left-context streaming.

    `lattice_tokens` is the smallest number of GPT latent tokens that may be dropped
    from the front of the vocoder input WITHOUT changing the sub-frame resampling
    phase of HifiDecoder's two internal `F.interpolate` stages
    (hifigan_decoder.py:85-96: x ar_mel_length_compression/output_hop_length, then
    x output_sample_rate/input_sample_rate). Dropping k tokens shifts the second
    interpolate's sampling grid by k * (1024/256) * (24000/22050) = 640k/147
    generator-input frames; gcd(640, 147) = 1, so this is a whole number of frames
    only when 147 | k. For any other k the generator is fed a fractionally
    resampled version of the same latent curve and -- being a conv net that
    synthesises phase, not a shift-equivariant filter -- emits a genuinely different
    waveform (E003 measured only -3..-8 dB residual-to-signal for non-lattice k,
    after an integer best-lag search, versus -81 dB for k = 147).

    Returns None if the decoder does not expose the rate attributes, in which case
    the caller must keep the original full-context behaviour.
    """
    try:
        comp = int(decoder.ar_mel_length_compression)
        hop = int(decoder.output_hop_length)
        sr_out = int(decoder.output_sample_rate)
        sr_in = int(decoder.input_sample_rate)
    except (AttributeError, TypeError, ValueError):
        return None
    if min(comp, hop, sr_in, sr_out) <= 0:
        return None
    ratio = Fraction(comp, hop) * Fraction(sr_out, sr_in)  # generator-input frames per token
    lattice = ratio.denominator  # 147 for the production rates
    frames_per_step = int(ratio * lattice)  # 640
    # the generator's total upsampling factor equals output_hop_length (8*8*2*2 = 256)
    return lattice, frames_per_step * hop  # (147, 163840)
...
def load_audio(audiopath, sampling_rate):
    ...
    audio_np, lsr = sf.read(audiopath, dtype="float32", always_2d=True)
    audio = torch.from_numpy(audio_np.T)  # soundfile: (frames, canales) -> torchaudio: (canales, frames)
...
class Xtts(BaseTTS):
    ...
    def handle_chunks(self, wav_gen, wav_gen_prev, wav_overlap, overlap_len,
                      wav_gen_abs_start=0, prev_total_len=None):
        """Handle chunk formatting in streaming mode

        wav_gen_abs_start: absolute sample index of ``wav_gen[0]`` in full-utterance
            coordinates. 0 when the whole utterance was re-vocoded (original behaviour).
        prev_total_len: absolute sample count produced by the previous pass. Defaults
            to ``wav_gen_prev.shape[0]``, which only equals it when
            ``wav_gen_abs_start == 0``. With both defaults this method behaves exactly
            as before (verified bit-identical, experiment E003).
        """
        if prev_total_len is None:
            prev_total_len = wav_gen_prev.shape[0] if wav_gen_prev is not None else 0
        start = prev_total_len - overlap_len - wav_gen_abs_start

        wav_chunk = wav_gen[:-overlap_len]
        if wav_gen_prev is not None:
            wav_chunk = wav_gen[start:-overlap_len]
        if wav_overlap is not None:
            # cross fade the overlap section
            if overlap_len > len(wav_chunk):
                # wav_chunk is smaller than overlap_len, pass on last wav_gen
                if wav_gen_prev is not None:
                    wav_chunk = wav_gen[start:]
                else:
    ...
    def inference_stream(
        ...
        **hf_generate_kwargs,
        ...
        # Bounded-left-context re-vocoding (E003). Only enabled at speed == 1.0, i.e.
        # the production path; the length_scale != 1.0 branch below resamples the
        # latents themselves and stays on the original full-context code path.
        ctx_lattice = _stream_ctx_lattice(self.hifigan_decoder) if length_scale == 1.0 else None
        ...
        for sent in text:
            ...
            # Pre-allocated KV-cache buffer (E018, 2026-08-29): avoids DynamicCache's default
            # per-step torch.cat reallocation (measured ~9.6% of total wall time via cProfile,
            # see .claude-work/experiments/E018_preallocated_kv_cache/). Sized exactly like
            # GPT.get_generator's own max_length computation below, so it can never overflow.
            # Bit-identical output verified vs the default DynamicCache path; does not touch
            # attention_mask handling (unlike cache_implementation="static", which broke XTTS's
            # custom positional-embedding logic -- see E014, abandoned).
            gpt_generate_kwargs = dict(hf_generate_kwargs)
            gpt_generate_kwargs.setdefault(
                "past_key_values",
                build_preallocated_cache(
                    self.gpt.gpt.config, max_cache_len=self.gpt.max_gen_mel_tokens + fake_inputs.shape[-1]
                ),
            )
            ...
            wav_overlap = None
            is_end = False
            # bounded-left-context state (E003)
            ctx_start = 0  # index of the first latent actually fed to the vocoder
            prev_total_len = 0  # absolute sample count produced by the previous pass
            bounded = ctx_lattice is not None
            ...
            while not is_end:
                ...
                if is_end or (stream_chunk_size > 0 and len(last_tokens) >= stream_chunk_size):
                    # Re-vocode only the new chunk plus a bounded left context, instead
                    # of the whole utterance from token 0. ctx_start is snapped to the
                    # resampling-phase lattice (see _stream_ctx_lattice) because any
                    # other trim changes the waveform the generator produces.
                    if bounded:
                        lattice, samples_per_step = ctx_lattice
                        min_window = max(stream_chunk_size, 1) + XTTS_STREAM_LEFT_CONTEXT_TOKENS
                        steps = max(0, (len(all_latents) - min_window) // lattice)
                        ctx_start = steps * lattice
                        abs_start = steps * samples_per_step
                    else:
                        ctx_start = 0
                        abs_start = 0
                    gpt_latents = torch.cat(all_latents[ctx_start:], dim=0)[None, :]
                    if length_scale != 1.0:
                        gpt_latents = F.interpolate(
                            gpt_latents.transpose(1, 2), scale_factor=length_scale, mode="linear"
                        ).transpose(1, 2)
                    wav_gen = self.hifigan_decoder(gpt_latents, g=speaker_embedding.to(self.device)).squeeze()
                    if bounded and wav_gen_prev is not None and (
                        prev_total_len - overlap_wav_len - abs_start < 0
                    ):
                        # Defensive: the slice would run off the front of this pass's
                        # output. Cannot happen with the lattice/min_window arithmetic
                        # above, but emitting the wrong region would be worse than
                        # paying for one more full re-synthesis.
                        logger.warning(
                            "bounded-context streaming slice out of range "
                            "(prev_total_len=%d abs_start=%d); falling back to full re-synthesis",
                            prev_total_len,
                            abs_start,
                        )
                        bounded = False
                        ...
                        wav_gen = self.hifigan_decoder(
                            gpt_latents, g=speaker_embedding.to(self.device)
                        ).squeeze()
                    wav_chunk, wav_gen_prev, wav_overlap = self.handle_chunks(
                        ...
                        wav_overlap,
                        overlap_wav_len,
                        wav_gen_abs_start=abs_start,
                        prev_total_len=prev_total_len,
                    )
                    prev_total_len = abs_start + wav_gen.shape[0]
                    last_tokens = []
                    yield wav_chunk
